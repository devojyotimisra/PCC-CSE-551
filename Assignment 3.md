...
BEGIN
    OPEN C;
    LOOP
        FETCH C INTO V_RADIUS, V_AREA ;
        DBMS_OUTPUT.PUT_LINE('RADIUS: ' || V_RADIUS || ', AREA: ' || V_AREA);
        EXIT WHEN C%NOTFOUND;
        V_COUNT:=V_COUNT+1;
    END LOOP;
    CLOSE C;
    DBMS_OUTPUT.PUT_LINE('TOTAL NO OF RECORS: ' || V_COUNT);
END;
/
...
BEGIN
    ...
    WHILE(TEMP1!=0)
    LOOP
        ANSWER:=ANSWER + POWER(MOD(TEMP1,10),C);
    ...
    ELSE
        DBMS_OUTPUT.PUT_LINE('NOT ARMSTORNG');
    END IF;
END;
/
...
BEGIN
    ...
    SELECT DECODE(SIGN((INPUT1*INPUT2)-100),0,'EQUAL TO 100',1,'GREATER THAN 100','LESS THAN 100') 
    INTO ANSWER 
    FROM DUAL;
...
BEGIN
    ...
    IF INSTR(STR1, STR2)=0 AND INSTR(STR2, STR1)=0
    THEN
...
BEGIN
    V_ACC_NO:='&V_ACC_NO';
    V_TYPE:='&V_TYPE';
    INSERT INTO ACCT_MSTR20231056 VALUES (V_ACC_NO, V_TYPE); 
    EXISTS_FLAG := F_CHKACCTNO(MACCT_NO);
    IF EXISTS_FLAG = 0 
    THEN
        SELECT 'T'||TO_CHAR(MAX(TO_NUMBER(SUBSTR(TRANS_NO, 2))) + 1) INTO V_NEXT_TRANS_NO FROM TRANS_MSTR20231056;
        IF MTYPE = 'SB'
        THEN
            INSERT INTO TRANS_MSTR20231056 VALUES (V_NEXT_TRANS_NO, MACCT_NO, SYSDATE, 'C', 'INITIAL PAYMENT', 'D', 500, 500);
        ELSE
            INSERT INTO TRANS_MSTR20231056 VALUES (V_NEXT_TRANS_NO, MACCT_NO, SYSDATE, 'C', 'INITIAL PAYMENT', 'D', 2000, 2000);
        END IF;
    END IF;
END;
/
